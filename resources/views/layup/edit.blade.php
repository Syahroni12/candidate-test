...
<main class="max-w-[800px] mx-auto px-7 py-10">
    ...
    {{-- Page Header --}}
    <div class="mb-8">
        <h1 class="text-[26px] font-serif font-bold text-gray-900 mb-1 tracking-tight">Edit Layup</h1>
        <p class="text-[14px] text-gray-500">Update layup record for <span class="font-semibold text-gray-700">{{ $supplier->name }}</span>.</p>
    </div>

    {{-- Form Card --}}
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <form action="{{ route('layup.update', $layup->id) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $layup->name) }}" required
